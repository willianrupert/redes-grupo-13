# Redes — Grupo 13 (CIn-UFPE)

Repositório do Grupo 13 para o projeto de reestruturação da rede dos laboratórios do CIn-UFPE, usando Cisco Packet Tracer.

- Documentação completa das entregas (E1-E4): [PROJETO.md](PROJETO.md)
- Bloco IP do grupo: `172.20.13.0/24`
- Arquivo de topologia atual: [topologia.pkt](topologia.pkt) (abrir no Cisco Packet Tracer)

## Entregas

| Entrega | Relatório | Status |
|---|---|---|
| E1 — Design Físico e Topologia | [docx](E1%20-%20redes%28grupo-13%29.docx) · [pdf](E1%20-%20redes%28grupo-13%29.pdf) | ✅ Entregue |
| E2 — Endereçamento VLSM | [docx](E2%20-%20redes%28grupo-13%29.docx) | 🟡 Em revisão |
| E3 — Segmentação (VLANs/DHCP) | — | ⏳ Pendente |
| E4 — Roteamento Inter-VLAN e Conectividade | — | ⏳ Pendente |

## Integrantes

- Amanda Trinity Gomes Nascimento
- Luiz Miguel Freitas da Silva
- Maria Luísa Brandão Amaral
- Mirella Laura Fontinelle Martins
- Willian Neves Rupert Jones
- Maria Eduarda Torres da Costa Lira

## Prazos

| Entrega | Data |
|---|---|
| E1 — Design Físico e Topologia | 04/09/2026 |
| E2 — Endereçamento VLSM | 16/09/2026 |
| E3 — Segmentação (VLANs/DHCP) | 25/09/2026 |
| E4 — Roteamento Inter-VLAN e Conectividade | 07/10/2026 |
